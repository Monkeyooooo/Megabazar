...
<body>
    ...
    <div>
<?php
$sql = "SELECT u.name, a.title, a.description, a.price, a.photo_link from users u INNER JOIN ads a on u.id = a.user_id where a.category_id=1;";
$result = $mysqli->query($sql);
while($row=mysqli_fetch_assoc($result)){
// echo '<div>'.$row['name'].' '.$row['title'].' '.$row['description'].' '.$row['price'].' '.'<img src="$row['photo_link']">'.'div>';
